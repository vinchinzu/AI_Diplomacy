...
class TestLLMAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self, MockDiplomacyAgentState, MockLLMPromptStrategy, MockLLMCoordinator, MockContextProviderFactory):
        ...
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "Default context", "tools_available": False, "provider_type": "inline", "tools": []}) # Added provider_type and tools
        self.mock_context_provider_factory.get_provider.return_value = self.mock_context_provider

        self.mock_llm_caller_override = AsyncMock(return_value={"orders": ["A PAR H"]}) # Changed to return dict
        ...
        self.agent.llm_caller_override = self.mock_llm_caller_override # Assign after agent init for specific tests

    async def test_initialization(self):
        # Re-init agent without override for this specific test to avoid interference
        agent = LLMAgent(
            agent_id="test_agent_init", country="FRANCE", config=self.agent_config,
            game_id="test_game_init", llm_coordinator=self.mock_llm_coordinator,
            context_provider_factory=self.mock_context_provider_factory, prompt_loader=self.mock_prompt_loader,
        ...
        for c in self.mock_prompt_loader.call_args_list:
            if c == call('system_prompt.txt') and self.mock_prompt_loader("france_system_prompt.txt") is None:
    ...
    async def test_load_system_prompt_power_specific(self):
        ...
        self.agent.llm_caller_override = None # Ensure no override for this part of init
        ...
        self.mock_prompt_loader.assert_any_call("france_system_prompt.txt")
        found_default_call = any(c == call('system_prompt.txt') for c in self.mock_prompt_loader.call_args_list)
    ...
    async def test_decide_orders_successful(self):
        ...
        mock_phase_state.phase_name = "Spring1901"
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "Test context", "tools_available": False, "provider_type": "inline", "tools": []})
    ...
    async def test_decide_orders_llm_error(self):
        ...
        mock_phase_state.get_power_units.return_value = ["A PAR", "F BRE"]
        mock_phase_state.phase_name = "Spring1901"
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "Test context", "provider_type": "inline", "tools_available": False, "tools": []})
        self.mock_prompt_strategy.build_order_prompt.return_value = "Test order prompt"
    ...
    async def test_decide_orders_context_tools_available(self):
        ...
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "Context with tools", "tools_available": True, "tools_definition": mock_tools, "provider_type": "inline", "tools": mock_tools})
    ...
    async def test_decide_orders_context_empty_text(self):
        ...
        mock_phase_state.get_power_units.return_value = ["A PAR"]
        mock_phase_state.phase_name = "Spring1901"
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "", "tools_available": False, "provider_type": "inline", "tools": []})
        self.mock_prompt_strategy.build_order_prompt.return_value = "Prompt with empty context"
        self.mock_llm_coordinator.call_json.return_value = {"orders": ["A PAR H"]}
        await self.agent.decide_orders(mock_phase_state)
        self.mock_prompt_strategy.build_order_prompt.assert_called_with(
            country="FRANCE",
    ...
    def test_extract_orders_from_response(self):
        ...
        temp_agent_config = AgentConfig(country="FRANCE", type="llm", model_id="dummy_model", context_provider="inline")
        agent = LLMAgent(
            agent_id="dummy_agent_extract", country="FRANCE", config=temp_agent_config,
            game_id="dummy_game", llm_coordinator=MagicMock(),
            context_provider_factory=MagicMock(), prompt_loader=MagicMock(), llm_caller_override=None
        )
        units = ["A PAR", "F BRE", "A MAR"] 
        response = {"orders": ["A PAR H", "F BRE M MAR", "INVALID ORDER STRING"]}
        expected_orders = [Order("A PAR H"), Order("F BRE M MAR"), Order("INVALID ORDER STRING")]
        actual_orders = agent._extract_orders_from_response(response, units)
        self.assertEqual(actual_orders, expected_orders)
        response_dict_order = {"orders": [{"unit": "A PAR", "action": "H"}, "F BRE M MAR"]}
        expected_orders_dict = [Order("A PAR H"), Order("F BRE M MAR")]
        actual_orders_dict = agent._extract_orders_from_response(response_dict_order, units)
        self.assertEqual(actual_orders_dict, expected_orders_dict)
        response_all_dict_order = {"orders": [{"unit": "A PAR", "action": "H"}, {"unit": "F BRE", "action": "M MAR"}]}
        expected_all_dict_orders = [Order("A PAR H"), Order("F BRE M MAR")]
        actual_all_dict_orders = agent._extract_orders_from_response(response_all_dict_order, units)
        self.assertEqual(actual_all_dict_orders, expected_all_dict_orders)
        response_partial_dict = {"orders": [{"action": "H"}, {"unit": "F BRE"}, "A MAR H"]}
        expected_partial_dict_orders = [Order("A MAR H")] 
        units_for_partial = ["X YZ", "F BRE", "A MAR"]
        actual_partial_dict_orders = agent._extract_orders_from_response(response_partial_dict, units_for_partial)
        self.assertEqual(actual_partial_dict_orders, expected_partial_dict_orders)

    def test_extract_orders_edge_cases(self):
        temp_agent_config = AgentConfig(country="FRANCE", type="llm", model_id="dummy_model", context_provider="inline")
        agent = LLMAgent(
            agent_id="dummy_agent_extract_edge", country="FRANCE", config=temp_agent_config,
            game_id="dummy_game_edge", llm_coordinator=MagicMock(),
            context_provider_factory=MagicMock(), prompt_loader=MagicMock(), llm_caller_override=None
        )
        units = ["A PAR"]
        ...
        self.assertEqual(agent._extract_orders_from_response({"orders": None}, ["A PAR"]), default_hold) # provide units list
        self.assertEqual(agent._extract_orders_from_response({"orders": []}, ["A PAR"]), default_hold) # provide units list
        self.assertEqual(agent._extract_orders_from_response({"orders": "not a list"}, units), default_hold)
        self.assertEqual(agent._extract_orders_from_response({"orders": [123, True]}, units), default_hold) # No valid orders, defaults to hold
        self.assertEqual(agent._extract_orders_from_response({"orders": [{"unit": "A PAR"}]}, units), default_hold) # Missing action
        self.assertEqual(agent._extract_orders_from_response({"orders": [{"action": "H"}]}, units), default_hold)   # Missing unit
        self.assertEqual(agent._extract_orders_from_response({"orders": [{"unit": 123, "action": "H"}]}, units), default_hold) # Invalid unit type
        self.assertEqual(agent._extract_orders_from_response({"orders": [{"unit": "A PAR", "action": True}]}, units), default_hold) # Invalid action type
    ...
    async def test_negotiate_successful(self):
        ...
        mock_phase_state.is_power_eliminated.side_effect = lambda p: False # No one eliminated
        expected_active_powers = ["ENGLAND", "GERMANY"] # FRANCE is self.country

        mock_phase_state.phase_name = "Spring1901Diplomacy"
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "Negotiation context", "tools_available": False, "provider_type": "inline", "tools": []})
        self.mock_prompt_strategy.build_negotiation_prompt.return_value = "Test negotiation prompt"
        self.mock_llm_coordinator.call_json.return_value = {"messages": [{"recipient": "ENGLAND", "content": "Hello England"}] }
        ...
        self.assertEqual(messages[0], Message("ENGLAND", "Hello England", message_type=constants.MESSAGE_TYPE_BROADCAST))
    ...
    async def test_negotiate_llm_error(self):
        ...
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "Context", "tools_available": False, "provider_type": "inline", "tools": []})
        self.mock_llm_coordinator.call_json.side_effect = Exception("LLM negotiation error")
        messages = await self.agent.negotiate(mock_phase_state)
    ...
    async def test_negotiate_context_tools_available(self):
        ...
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "Negotiation context with tools", "tools_available": True, "tools_definition": mock_tools, "provider_type": "inline", "tools": mock_tools})
        self.mock_prompt_strategy.build_negotiation_prompt.return_value = "Negotiation prompt with tools"
        self.mock_llm_coordinator.call_json.return_value = {"messages": [{"recipient": "ENGLAND", "content": "Tool talk"}]}
    ...
    async def test_negotiate_context_empty_text(self):
        ...
        mock_phase_state.phase_name = "Spring1901Diplomacy"
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "", "tools_available": False, "provider_type": "inline", "tools": []})
        self.mock_prompt_strategy.build_negotiation_prompt.return_value = "Negotiation prompt with empty context"
        self.mock_llm_coordinator.call_json.return_value = {"messages": [{"recipient": "ENGLAND", "content": "Empty context talk"}]}
        await self.agent.negotiate(mock_phase_state)
        self.mock_prompt_strategy.build_negotiation_prompt.assert_called_with(
            country="FRANCE",
            active_powers=expected_active_powers,
    ...
    def test_extract_messages_from_response(self):
        temp_agent_config = AgentConfig(country="FRANCE", type="llm", model_id="dummy_model", context_provider="inline")
        agent = LLMAgent(
            agent_id="dummy_agent_extract_msg", country="FRANCE", config=temp_agent_config,
            game_id="dummy_game_msg", llm_coordinator=MagicMock(),
            context_provider_factory=MagicMock(), prompt_loader=MagicMock(), llm_caller_override=None
        )
        ...
        response = {"messages": [
            ...
            {"recipient": "GERMANY", "content": "Hi Germany", "message_type": "SECRET"},
            {"recipient": "ITALY", "content": "Ciao Italy"} 
        ]}
        expected_messages = [
            Message("ENGLAND", "Hello England", message_type=constants.MESSAGE_TYPE_BROADCAST), 
            Message("GERMANY", "Hi Germany", message_type="SECRET"),
            Message("ITALY", "Ciao Italy", message_type=constants.MESSAGE_TYPE_BROADCAST) 
        ]
        actual_messages = agent._extract_messages_from_response(response, mock_phase_state)
        self.assertEqual(actual_messages, expected_messages)
        response_invalid_type = {"messages": [
            {"recipient": "ENGLAND", "content": "Type test", "message_type": "INVALID_TYPE"}
        ]}
        expected_messages_invalid_type = [
            Message("ENGLAND", "Type test", message_type=constants.MESSAGE_TYPE_BROADCAST) 
        ]
        actual_messages_invalid_type = agent._extract_messages_from_response(response_invalid_type, mock_phase_state)
        self.assertEqual(actual_messages_invalid_type, expected_messages_invalid_type)

    def test_extract_messages_edge_cases(self):
        temp_agent_config = AgentConfig(country="FRANCE", type="llm", model_id="dummy_model", context_provider="inline")
        agent = LLMAgent(
            agent_id="dummy_agent_extract_msg_edge", country="FRANCE", config=temp_agent_config,
            game_id="dummy_game_msg_edge", llm_coordinator=MagicMock(),
            context_provider_factory=MagicMock(), prompt_loader=MagicMock(), llm_caller_override=None
        )
        mock_phase_state = MagicMock(spec=PhaseState)
        mock_phase_state.powers = frozenset(["ENGLAND", "GERMANY", "ITALY", "RUSSIA", "AUSTRIA", "TURKEY", "FRANCE"])
        self.assertEqual(agent._extract_messages_from_response(None, mock_phase_state), [])
        self.assertEqual(agent._extract_messages_from_response({}, mock_phase_state), [])
        self.assertEqual(agent._extract_messages_from_response({"messages": None}, mock_phase_state), [])
        self.assertEqual(agent._extract_messages_from_response({"messages": []}, mock_phase_state), [])
        self.assertEqual(agent._extract_messages_from_response({"messages": "not a list"}, mock_phase_state), [])
        self.assertEqual(agent._extract_messages_from_response({"messages": [123]}, mock_phase_state), [])
        self.assertEqual(agent._extract_messages_from_response({"messages": [{}]}, mock_phase_state), []) 
        self.assertEqual(agent._extract_messages_from_response({"messages": [{"recipient": "ENGLAND"}]}, mock_phase_state), []) 
        self.assertEqual(agent._extract_messages_from_response({"messages": [{"content": "Hi"}]}, mock_phase_state), []) 
        self.assertEqual(agent._extract_messages_from_response({"messages": [{"recipient": 123, "content": "Hi"}]}, mock_phase_state), []) 
        self.assertEqual(agent._extract_messages_from_response({"messages": [{"recipient": "ENGLAND", "content": True}]}, mock_phase_state), []) 
        self.assertEqual(agent._extract_messages_from_response({"messages": [{"recipient": "SPAIN", "content": "Hola"}]}, mock_phase_state), []) 
        response_non_string_type = {"messages": [
            {"recipient": "ENGLAND", "content": "Type test", "message_type": 123}
        ]}
        expected_msg_non_string_type = [Message("ENGLAND", "Type test", message_type=constants.MESSAGE_TYPE_BROADCAST)]
        actual_msg_non_string_type = agent._extract_messages_from_response(response_non_string_type, mock_phase_state)
    ...
    async def test_update_state(self):
        ...
        mock_phase_state.get_center_count.side_effect = lambda p: 2 if p == "FRANCE" else (3 if p == "GERMANY" else 0)
        mock_events = [{"type": "build", "power": "FRANCE", "details": "A PAR built"}] 
        self.mock_llm_coordinator.call_json.return_value = {"diary_entry": "Test diary entry"}
    ...
    async def test_generate_phase_diary_entry_successful(self):
        ...
        self.mock_llm_coordinator.call_json.return_value = {constants.LLM_RESPONSE_KEY_DIARY_ENTRY: "LLM diary entry"}
        return_value = await self.agent._generate_phase_diary_entry(mock_phase_state, mock_events)
    ...
    async def test_generate_phase_diary_entry_llm_error(self):
        ...
        mock_phase_state.is_game_over.return_value = False
        self.mock_llm_coordinator.call_json.side_effect = Exception("LLM error for diary")
    ...
    async def test_analyze_and_update_goals_change(self):
        ...
        mock_phase_state.get_center_count.side_effect = lambda p: 2 if p == "FRANCE" else 5

        initial_goals = ["Initial Goal To Be Changed Different From Rule"] # Make sure it's different
    ...
    async def test_analyze_and_update_goals_clear_leader(self):
        ...
        mock_phase_state.get_center_count.side_effect = lambda p: 10 if p == "FRANCE" else 1
        mock_phase_state.is_power_eliminated.return_value = False
    ...
    async def test_analyze_and_update_goals_no_redundant_addition(self):
        ...
        mock_phase_state.get_center_count.side_effect = lambda p: 5 if p == "FRANCE" else 3
        mock_phase_state.powers = frozenset(["FRANCE", "GERMANY"])
        mock_phase_state.is_power_eliminated.return_value = False
    ...
    async def test_analyze_and_update_goals_no_change(self):
        ...
        mock_phase_state.get_center_count.side_effect = lambda p: 6 if p == "FRANCE" else 3
        mock_phase_state.powers = frozenset(["FRANCE", "GERMANY"])
        mock_phase_state.is_power_eliminated.return_value = False
    ...
    async def test_llm_caller_override_usage(self):
        ...
        self.mock_context_provider.provide_context = AsyncMock(return_value={"context_text": "Context for override", "tools_available": False, "provider_type": "inline", "tools": []})
        self.mock_prompt_strategy.build_order_prompt.return_value = "Order prompt for override"
    ...
    async def test_agent_with_various_context_providers(self):
        ...
        logger.info("Testing agents with context providers (ported from test_stage2)...")
        ...
        factory = AgentFactory(llm_coordinator=self.mock_llm_coordinator, context_provider_factory=self.mock_context_provider_factory)
        ...
        mcp_agent = factory.create_agent("mcp-test-ctx", "GERMANY", mcp_config, game_id_context_test)
        auto_agent = factory.create_agent("auto-test-ctx", "ENGLAND", auto_config, game_id_context_test)
