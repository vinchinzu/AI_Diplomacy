...
def test_game_history_to_dict_serialization():
    ...
    json_string = json.dumps(history_dict, indent=2) # Add indent for easier debugging if it fails
...
def test_phase_add_orders_internal_consistency():
    ...
    assert phase_dict["results_by_power"]["FRANCE"] == [["SUCCESSFUL"], ["BOUNCED", "F ENG - MAO"]]
    ...
    assert gh_dict["phases"][0]["orders_by_power"]["FRANCE"] == ["A PAR H", "F BRE - MAO"]
    assert gh_dict["phases"][0]["results_by_power"]["FRANCE"] == [["SUCCESSFUL"], ["BOUNCED", "F ENG - MAO"]]
