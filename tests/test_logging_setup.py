...
from ai_diplomacy.logging_setup import get_log_paths, JsonFormatter, setup_logging, LLMVerboseFilter
from ai_diplomacy.game_config import GameConfig # Needed for setup_logging and GameConfig tests

# Helper to create a dummy GameConfig for tests
def make_dummy_game_config(log_level="INFO", log_to_file=False, general_log_path=None, verbose_llm_debug=False):
    # Mock argparse.Namespace
    args = mock.Mock()
    args.log_level = log_level
    args.log_to_file = log_to_file # This will be used by GameConfig's logic
    args.general_log_path = general_log_path if general_log_path else "dummy_general.log" # Path for GameConfig internal use
...
class TestGetLogPaths(unittest.TestCase):
    def test_get_log_paths_structure_and_content(self):
        ...
        expected_llm_log_path = os.path.join(expected_game_id_specific_log_dir, f"{game_id}_llm_interactions.csv")
        expected_general_log_path = os.path.join(expected_game_id_specific_log_dir, f"{game_id}_general.log")
        expected_results_dir = os.path.join(expected_game_id_specific_log_dir, "results")
        ...
        self.assertEqual(paths["game_id_specific_log_dir"], expected_game_id_specific_log_dir)
...
class TestJsonFormatter(unittest.TestCase):
    ...
    def test_basic_log_formatting(self):
        ...
        self.assertIn('timestamp', log_output) # Check presence, exact value depends on Formatter's asctime
    ...
    def test_unserializable_extra_field_fallback(self):
        ...
        if "extra" in log_output and "unserializable" in log_output["extra"]:
            self.assertIsInstance(log_output['extra']['unserializable'], str) # if it converts to str
        # else:
...
class TestSetupLoggingFormatSelection(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_game_config = make_dummy_game_config(log_to_file=False) # Don't need file handler for this test
    ...
    def test_setup_logging_uses_json_formatter_when_env_set(self):
        ...
        for handler in self.root_logger.handlers:
            if isinstance(handler.formatter, JsonFormatter):
                ...
                break
        self.assertTrue(found_json_formatter, "JsonFormatter should be used when LOG_FORMAT=JSON")
    ...
    def test_setup_logging_uses_default_formatter_when_env_not_set(self):
        ...
        for handler in self.root_logger.handlers:
            ...
            if isinstance(handler.formatter, logging.Formatter) and \
               not isinstance(handler.formatter, JsonFormatter):
                found_default_formatter = True
                break
        self.assertTrue(found_default_formatter, "Default Formatter should be used when LOG_FORMAT is not JSON")
