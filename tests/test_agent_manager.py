    ...
    def __init__(
        ...
        class MockArgs:
            def __init__(self):
                ...
                self.exclude_powers = (
                    exclude_powers if exclude_powers is not None else []
                )
        ...
        self.power_model_assignments = (
            power_model_assignments if power_model_assignments is not None else []
        )
...
def test_basic_assignment_two_llm_players_no_primary():
    ...
    config1 = MockGameConfig(
        num_players=2, fixed_models=["ollama/modelA", "ollama/modelB"]
    )
...
def test_primary_agent_specified_three_llm_players():
    ...
    config2 = MockGameConfig(
        num_players=3,
        power_name="FRANCE",
        model_id="gpt-4o",
        fixed_models=["ollama/modelC"],
    )
...
def test_exclude_powers_randomize_fixed_models():
    ...
    config3 = MockGameConfig(
        num_players=2,
        fixed_models=["modelX", "modelY", "modelZ"],
        randomize_fixed_models=True,
        exclude_powers=["ITALY", "TURKEY"],
    )
...
def test_not_enough_fixed_models_for_num_players():
    ...
    assert (
        models_assigned.count("only_one_model") > 0
        or models_assigned.count(DEFAULT_AGENT_MANAGER_FALLBACK_MODEL) > 0
    )
...
def test_primary_agent_excluded():
    ...
    config = MockGameConfig(
        num_players=1, power_name="FRANCE", model_id="gpt-4o", exclude_powers=["FRANCE"]
    )
...
def test_toml_and_cli_primary_agent_conflict_cli_wins():
    ...
    config = MockGameConfig(
        num_players=1,
        power_name="FRANCE",
        model_id="cli_model_F",
        power_model_assignments=toml_assignments,
    )
    manager = AgentManager(config)
    assigned = manager.assign_models(ALL_POWERS_IN_GAME)
...
def test_default_model_from_config_used():
    ...
    config = MockGameConfig(
        num_players=1, default_model_from_config="toml_default_model"
    )
    manager = AgentManager(config)
    assigned = manager.assign_models(ALL_POWERS_IN_GAME)
...
def test_fallback_model_used_if_no_config_default():
    ...
    config = MockGameConfig(
        num_players=1
    )  # default_model_from_config is None by default in MockGameConfig
    manager = AgentManager(config)
    assigned = manager.assign_models(ALL_POWERS_IN_GAME)
    ...
    assigned_model = list(assigned.values())[0]
    assert (
        assigned_model == DEFAULT_AGENT_MANAGER_FALLBACK_MODEL
