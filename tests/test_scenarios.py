...
def test_scenario_registry_populated():
    ...
    for name, func in expected_scenarios.items():
        ...
        assert SCENARIO_REGISTRY[name] == func, (
            f"Registry entry for '{name}' does not point to the correct function."
        )
    ...
    for name, factory_func in SCENARIO_REGISTRY.items():
        ...
        if name == "wwi_two_player":
            ...
            game_instance = factory_func(
                entente_player="ENTENTE_POWERS", central_player="CENTRAL_POWERS"
            )
            ...
            try:
                ...
                pytest.fail(
                    f"Scenario '{name}' factory function could not be called without arguments. "
...
def test_wwi_two_player_specifics():
    ...
    assert "standard" in game.variant.name.lower(), (
        "wwi_two_player should use a standard variant base."
    )
