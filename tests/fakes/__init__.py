...
class LoggingMixin:
    def __init__(self, logger_name=None, **kwargs): # Added **kwargs to play nice with super() in multiple inheritance
...
class GameConfigAttributesMixin:
    def __init__(self, log_level="DEBUG", game_id="test_game_mixin", log_to_file=True, log_dir=None, **kwargs):
...
class MockGameHistory_PhaseSummary:
    ...
    ) -> Optional[MockPhase_PhaseSummary]: 
...
class MockArgs_LoggingSetup(GameConfigAttributesMixin):
    def __init__(
        ...
        super().__init__(log_level=log_level, game_id=game_id, log_to_file=log_to_file, log_dir=log_dir)
