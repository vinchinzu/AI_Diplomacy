...
def run_wwi_2p_test():
    ...
    command = [
        ...
        "lm_game.py",
        "--preset", "wwi_2p",
        "--players", "llm,llm", # Two LLM agents controlling the blocs
        "--llm-models", "gemma3:4b,gemma3:4b", # Both blocs use gemma3:4b
        "--log_level", "INFO", # Example: Set a log level
    ...
    try:
        ...
        if process.returncode == 0 and not has_errors:
            ...
            print(f"❌ WWI 2-player test failed. Return code: {process.returncode}. Check output for ERROR messages.")
            # Output was already streamed
            
    except FileNotFoundError:
        print("Error: lm_game.py not found. Make sure it's in the same directory or in PYTHONPATH.")
        sys.exit(1)
