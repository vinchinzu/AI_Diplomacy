...
async def initialize_agent_state_ext(
    agent: "BaseAgent", game: "Game", game_history: "GameHistory", log_file_path: str
):
    """
    ...
    logger.warning(
        f"initialize_agent_state_ext() is deprecated. Agent {power_name} initialization should be handled by the agent itself."
    )
...
async def initialize_agents_concurrently(
    ...
    """
    logger.warning(
        "initialize_agents_concurrently() is deprecated. Use the new agent factory system instead."
    )
    return
