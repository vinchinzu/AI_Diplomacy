...
if __name__ == "__main__":
    ...
    class MockLLMInterface:
        ...
        async def generate_phase_result_diary(
            self, prompt_template_vars, log_file_path, game_phase
        ):
            self.logger.info(
                f"generate_phase_result_diary called for {game_phase} with vars: {list(prompt_template_vars.keys())}"
            )
    ...
    class MockGameHistory:
        ...
        def get_phase_by_name(self, name_to_find: str) -> Optional[MockPhase]:
            ...
            logger.info(
                f"[MockGameHistory] Auto-added phase {name_to_find} for summary testing."
            )
            return new_phase

        def get_messages_by_phase(
            self, phase_name: str
        ) -> List[Any]:  # Return list of mock messages
        ...
        def add_phase_summary(self, phase_name: str, power_name: str, summary: str):
            ...
            if phase:
                ...
                logger.error(
                    f"[MockGameHistory] Phase {phase_name} not found to add summary for {power_name}"
                )
    ...
    class MockGameConfig(GameConfig):
        def __init__(self, power_name="FRANCE"):
            ...
            self.agents = (
                {}
            )  # To store mock agents if needed for goal/relationship fetching
    ...
    async def run_summary_generator_test():
        ...
        mock_game = MockGame(
            current_phase_name="AUTUMN 1901M"
        )  # Phase after the one being summarized
        mock_history = MockGameHistory()

        phase_to_summarize = "SPRING 1901M"
        # Add some history for the phase being summarized
        spring_1901_phase = mock_history.get_phase_by_name(
            phase_to_summarize
        ...
        all_orders_for_spring_1901 = (
            spring_1901_phase.orders_by_power if spring_1901_phase else {}
        )
        ...
        if recorded_phase and recorded_phase.phase_summaries.get(power_name_test):
            logger.info(
                f"Summary for {power_name_test} correctly recorded in history for {phase_to_summarize}: {recorded_phase.phase_summaries[power_name_test][:50]}..."
            )
        else:
            logger.error(
                f"Summary for {power_name_test} NOT recorded in history for {phase_to_summarize}."
            )
