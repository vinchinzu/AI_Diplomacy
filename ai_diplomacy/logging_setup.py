...
if __name__ == "__main__":
    ...
    class MinimalGameConfig:
        def __init__(
            ...
            self.general_log_path = os.path.join(
                log_dir if log_dir else ".", f"{game_id}_general.log"
            )
    ...
    config1 = MinimalGameConfig(
        log_level="DEBUG", game_id="log_test_verbose_false", verbose_llm_debug=False
    )
    setup_logging(config1)  # type: ignore
    logging.getLogger("ai_diplomacy.llm_coordinator").info(
        "LLM Coordinator Prompt: This is a very long prompt..."
    )
    logging.getLogger("ai_diplomacy.other_module").info("Other Info: Regular message.")
    logging.getLogger("ai_diplomacy.llm_coordinator").debug(
        "LLM Coordinator DEBUG: Full details here."
    )

    # Test 2: Logging to file and console, verbose_llm_debug = True
    print("\n--- Test 2: Logging (DEBUG level), verbose_llm_debug = True ---")
    config2 = MinimalGameConfig(
        log_level="DEBUG", game_id="log_test_verbose_true", verbose_llm_debug=True
    )
    ...
    config3 = MinimalGameConfig(
        log_level="INFO", game_id="log_test_info_verbose_false", verbose_llm_debug=False
    )
    ...
    logging.getLogger("ai_diplomacy.llm_coordinator").debug(
        "LLM Coordinator DEBUG: This should not appear."
    )
