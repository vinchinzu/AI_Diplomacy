...
def _extract_moves_from_llm_response(
    raw_response: str, power_name: str, model_id: str
) -> Optional[List[str]]:
    """
    Attempt multiple parse strategies to find JSON array of moves.
    """
    logger.debug(
        f"[{model_id}] Attempting to extract moves for {power_name} from raw response: {raw_response[:300]}..."
    )
    ...
    if not matches:
        logger.debug(
            f"[{model_id}] Regex for 'PARSABLE OUTPUT:' failed for {power_name}. Trying alternative patterns."
        )
    ...
    if not matches:  # Check for triple-backtick code fences
        logger.debug(
            f"[{model_id}] Regex for inline 'PARSABLE OUTPUT' failed. Trying triple-backtick code fences for {power_name}."
        )
    ...
    try:
        ...
        logger.warning(
            f"[{model_id}] JSON decode failed for {power_name}: {e}. JSON text was: '{json_text}'. Trying bracket fallback."
        )
...
def _validate_extracted_orders(
    ...
    for move_str in moves:
        if (
            not move_str or not isinstance(move_str, str) or move_str.strip() == ""
        ):  # Skip empty or non-string moves
            continue
        ...
        if move_str in all_possible_orders_set:
            ...
            if len(tokens) >= 2:
                ...
                if expected_unit not in power_units:
                    logger.warning(
                        f"[{model_id}] {power_name} tried to order unit '{expected_unit}' but doesn't control it. {power_name} units: {power_units}"
                    )
                else:
                    logger.warning(
                        f"[{model_id}] {power_name} has unit '{expected_unit}' but order '{move_str}' is not in possible orders for that unit."
                    )

    if invalid_moves_found:
        logger.info(
            f"[{model_id}] Some LLM-proposed moves for {power_name} were invalid. Invalid: {invalid_moves_found}"
        )
        if dev_mode:
        ...
        for loc, orders_list in possible_orders.items():
            ...
            if unit_loc_prefix not in used_locs and orders_list:
                ...
                if hold_candidates:
                    ...
                    logger.debug(
                        f"[{model_id}] Added first available order for unassigned unit at {loc} for {power_name}."
                    )
