        ...
        for power_name in powers_with_builds:
            ...
            if isinstance(agent, BlocLLMAgent):
                if agent.agent_id in processed_bloc_agent_ids:
                    ...
                    logger.debug(f"Processing BlocLLMAgent {agent.agent_id} for builds involving {power_name}...")
                    try:
                        ...
                        for bloc_power_name, order_obj_list in all_bloc_orders_obj.items():
                            if bloc_power_name in powers_with_builds: # Only add if this power actually has builds
                                ...
                                logger.debug(f"✅ {bloc_power_name} (Bloc): Generated {len(orders_str_list)} build/disband orders")
                        processed_bloc_agent_ids.add(agent.agent_id)
                    except Exception as e:
                        logger.error(
                            f"❌ Error getting build orders for bloc agent {agent.agent_id}, power {power_name}: {e}", exc_info=e
                        )
                        # Handle errors for bloc members needing builds
                        for bloc_member_power in agent.get_bloc_member_powers():
                            if bloc_member_power in powers_with_builds and bloc_member_power not in orders_by_power:
                                orders_by_power[bloc_member_power] = []
                                game_history.add_orders(current_phase_name, bloc_member_power, []) # Record empty due to error
