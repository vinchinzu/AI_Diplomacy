...
def assign_models_to_powers(game_config: "GameConfig", all_game_powers: List[str]) -> Dict[str, str]:
    ...
    default_model = game_config.default_model_from_config or DEFAULT_AGENT_MANAGER_FALLBACK_MODEL
    ...
    primary_agent_model_cli = game_config.model_id  # Model specified via CLI for primary agent
    ...
    num_additional_llm_players_needed = game_config.num_players - num_llm_controlled_so_far

    # Consider powers from TOML that are not excluded for this calculation
    candidate_powers_for_filling_slots = [
        p for p in all_game_powers if p not in exclude_powers and p not in current_llm_powers
    ]
    ...
    fixed_models_cli_list = list(game_config.fixed_models) if game_config.fixed_models else []
