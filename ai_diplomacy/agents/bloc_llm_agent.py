...
class BlocLLMAgent(LLMAgent):
    ...
    async def decide_orders(self, phase: PhaseState) -> List[Order]:
        ...
        units_by_power = phase.board.units
        if units_by_power:
        ...
        if self._cached_bloc_orders_phase_key == current_phase_key:
            logger.debug(
                f"Using cached bloc orders for phase key elements: year={phase.key.year}, season={phase.key.season}, name={phase.name}"
            )
        else:
            logger.info(
                f"New phase or state detected for bloc {self.bloc_name} (key elements: year={phase.key.year}, season={phase.key.season}, name={phase.name}), querying LLM for bloc orders."
            )
            ...
            # This method does not exist yet on PhaseState
                ...
                for power, orders_str_list in parsed_llm_orders.items():
                    ...
                    if power.upper() in self.controlled_powers:
                        if isinstance(orders_str_list, list) and all(
                            ...
                            valid_parsed_orders[power.upper()] = [Order(value=o) for o in orders_str_list]
                        else:
                            logger.warning(
    ...
    def get_all_bloc_orders_for_phase(
        ...
        """
        # This check should be safe even if self._cached_bloc_orders_phase_key is None
        if self._cached_bloc_orders_phase_key == phase_key_tuple:
            return self._cached_bloc_orders_this_phase
        else:
            logger.warning(
                f"Request for bloc orders with mismatched phase key. Requested: {phase_key_tuple}, "
                f"Cached: {self._cached_bloc_orders_phase_key}. Returning empty. "
                "Ensure decide_orders is called before get_all_bloc_orders_for_phase."
            )
            return {}
