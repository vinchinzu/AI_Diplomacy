...
from ..services.config import AgentConfig, DiplomacyConfig # DiplomacyConfig might not be used here directly anymore
...
class AgentFactory:
    ...
    def create_agent(
        ...
        """
        logger.info(f"Attempting to create agent of type '{config.type}' for '{country if config.type != 'bloc_llm' else bloc_name}' with ID {agent_id}")
        ...
        if config.type == "llm":
            ...
            if not bloc_name or not controlled_powers:
                raise ValueError("BlocLLMAgent requires bloc_name and controlled_powers.")
            ...
            return self._create_bloc_llm_agent(agent_id, bloc_name, controlled_powers, config, game_id)
        else:
    ...
    def _create_scripted_agent(
        ...
        personality = getattr(config, "personality", "neutral_hold") # Default to neutral_hold
    ...
    def _create_neutral_agent(
        self, agent_id: str, country: str, config: AgentConfig # Config might be minimal for Neutral
    ) -> NeutralAgent:
    ...
    def _create_bloc_llm_agent(
        ...
        if not config.model_id:
            raise ValueError(f"BlocLLMAgent for {bloc_name} requires model_id in config")
        if not controlled_powers:
            raise ValueError(f"BlocLLMAgent for {bloc_name} requires a list of controlled_powers.")

        logger.debug(f"Creating BlocLLMAgent for bloc '{bloc_name}' controlling {controlled_powers} with model {config.model_id}")
        return BlocLLMAgent(
    ...
    def create_agents_from_config(
        ...
        """
        agents = {}
        logger.warning("create_agents_from_config may need updates for bloc agent creation logic if used directly with complex configs.")

        for agent_config in diplomacy_config.agents: # agent_config is of type AgentConfig
            try:
                ...
                agent_id_country_part = agent_config.country # Fallback for single powers
                ...
                if agent_config.type == "bloc_llm":
                    logger.error(f"create_agents_from_config cannot create bloc_llm for {agent_config.country} without bloc_name and controlled_powers. Skipping.")
                    continue
                ...
                agents[agent_config.country] = agent # Keying by country might be an issue for blocs
                logger.info(f"Created agent for {agent_config.country} via create_agents_from_config")
        ...
        logger.info(f"Created {len(agents)} agents from configuration via create_agents_from_config")
        return agents
    ...
    def validate_agent_config(self, config: AgentConfig) -> bool:
        ...
        """
        try:
            if not config.country and config.type != "bloc_llm": # country is not primary for bloc, bloc_name is
                logger.error(
                    f"Agent config missing required field: country (for non-bloc types)"
                )
                return False
            if not config.type:
            ...
            if config.type not in ["llm", "scripted", "neutral", "bloc_llm"]: # Added new types
            ...
            if config.type == "bloc_llm" and not config.model_id:
                ...
                logger.error(f"BlocLLMAgent missing model_id in config")
                return False
