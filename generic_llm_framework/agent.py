...
class GenericLLMAgent(GenericLLMAgentInterface):
    ...
    def __init__(self, agent_id: str, config: Any, llm_coordinator: LLMCoordinator, prompt_strategy: BasePromptStrategy):
        """
    ...
    async def decide_action(self, state: Any, possible_actions: Any) -> Any:
        ...
        """
        logger.debug(f"Agent {self.agent_id}: Deciding action. State: {state}, Possible Actions: {possible_actions}")
        try:
            prompt_context = {
                'state': state,
                'possible_actions': possible_actions,
                'internal_state': self._internal_state, # Provide internal state to prompt strategy
            }
            ...
            system_prompt = self.config.get('system_prompt', self.prompt_strategy.system_prompt_template) # Use strategy's generic system prompt if none in config
            game_id = self.config.get('game_id', generic_constants.DEFAULT_GAME_ID)
            phase = self.config.get('phase', 'decide_action') # Or use a more specific phase from state if available
    ...
    async def generate_communication(self, state: Any, recipients: Any) -> Any:
        ...
        """
        logger.debug(f"Agent {self.agent_id}: Generating communication. State: {state}, Recipients: {recipients}")
        try:
            prompt_context = {
                'state': state,
                'recipients': recipients,
                'internal_state': self._internal_state, # Provide internal state
            }
            prompt = self.prompt_strategy.build_prompt(
            ...
            game_id = self.config.get('game_id', generic_constants.DEFAULT_GAME_ID)
            phase = self.config.get('phase', 'generate_communication') # Or use a more specific phase from state
            verbose_llm_debug = self.config.get('verbose_llm_debug', False)

            logger.debug(f"Agent {self.agent_id}: Calling LLM for communication generation. Model: {model_id}")
            response_json = await self.llm_coordinator.call_json(
                prompt=prompt,
                model_id=model_id,
                agent_id=self.agent_id,
            ...
            logger.error(f"Agent {self.agent_id}: Error during generate_communication: {e}", exc_info=True)
            return {"error": str(e), "details": "Failed to generate communication via LLM."}
    ...
    async def update_internal_state(self, state: Any, events: List[Dict[str, Any]]) -> None:
        ...
        """
        logger.info(f"Agent {self.agent_id}: Updating internal state. Current env state: {state}, Events: {events}")
        ...
        self._internal_state['last_updated'] = logger.name # Using logger.name as a placeholder for a timestamp or version
    ...
    def get_agent_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "internal_state_summary": {k: type(v).__name__ for k,v in self._internal_state.items()} # Summary of state keys and types
        }
